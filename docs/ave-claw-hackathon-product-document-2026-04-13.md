...
| `Browse` - signals | Public signal browser | Browse signal list; open token detail; cycle signal chain | `UP/DOWN`: move selection<br>`RIGHT` or `A`: open selected token in `Spotlight`<br>`X`: keep signal browsing on `solana`<br>`LEFT` or `B`: return to `Explorer`<br>`Y`: open `Portfolio` globally<br>`FN`: voice wake / PTT |
| `Browse` - watchlist | Saved-token browser | Browse watchlist; open token detail; cycle watchlist chain | `UP/DOWN`: move selection<br>`RIGHT` or `A`: open selected token in `Spotlight`<br>`X`: keep watchlist browsing on `solana`<br>`LEFT` or `B`: return to `Explorer`<br>`Y`: open `Portfolio` globally<br>`FN`: voice wake / PTT |
