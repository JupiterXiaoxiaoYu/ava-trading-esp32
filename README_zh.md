...
这个仓库是 Ava Box 硬件、后端和模拟器的一体化主仓库。

Ava Box 是一个语音驱动的交易助手，建立在 XiaoZhi 系固件、Python 后端以及可同时运行在硬件和桌面模拟器上的共享 LVGL 界面层之上。当前产品重点是 Scratch Arcade 风格的 ESP32-S3 板卡：320x240 屏幕、摇杆输入、语音唤醒，以及 Ava Box 的 feed、spotlight、portfolio、watchlist 和订单等页面。
...
- `simulator/` - 共享 Ava Box 界面的桌面验证环境和 mock 交互环境
- `docs/` - 当前仍保留的产品/参考文档
...
- 做服务端 Ava Box 行为相关工作：先看 [`server/README_en.md`](server/README_en.md) 和 [`server/main/README_en.md`](server/main/README_en.md)
- 在桌面上预览或调试 Ava Box 页面：先看 [`simulator/README.md`](simulator/README.md) 和 [`shared/ave_screens/README.md`](shared/ave_screens/README.md)
- 理解共享 Ava Box UI 层：先看 [`shared/README.md`](shared/README.md)
...
speech + input
  ...
  -> server/main/xiaozhi-server/ (ASR、路由、工具、Ava Box 后端逻辑)
...
- [`shared/README.md`](shared/README.md) - 共享跨端 Ava Box UI 层说明
- [`shared/ave_screens/README.md`](shared/ave_screens/README.md) - 页面文件、manager、工具和扩展方式
- [`simulator/README.md`](simulator/README.md) - Ava Box 模拟器的桌面构建/运行入口

### 设备运行时

- [`firmware/README.md`](firmware/README.md) - 上游固件能力说明 + Ava Box 单仓库补充说明
- [`firmware/main/README.md`](firmware/main/README.md) - 固件运行时内部结构、板级适配、显示/音频/协议入口

### 后端栈

- [`server/README_en.md`](server/README_en.md) - 带 Ava Box 单仓库语境的后端部署总览
