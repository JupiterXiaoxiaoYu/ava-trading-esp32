...
void Application::Initialize() {
    ...
    ESP_LOGI(TAG, "Initialize complete");
}

void Application::Run() {
    // Set the priority of the main task to 10
    vTaskPrioritySet(nullptr, 10);
    ESP_LOGI(TAG, "Run loop started");
...
void Application::InitializeProtocol() {
    ...
    protocol_->OnIncomingJson([this, display](const cJSON* root) {
        ...
        if (strcmp(type->valuestring, "tts") == 0) {
            ...
            if (raw != nullptr) {
                std::string raw_json(raw);
                cJSON_free(raw);
                Schedule([this, display, raw_json = std::move(raw_json)]() {
                    DisplayLockGuard lock(display);
                    if (!ave_display_initialized_) {
                        ave_sm_init(lv_display_get_default());
                        ave_display_initialized_ = true;
                    }
                    ave_sm_handle_json(raw_json.c_str());
                });
            }
...
void Application::HandleStopListeningEvent() {
    ...
    if (state == kDeviceStateAudioTesting) {
        ...
        ESP_LOGI(TAG, "Ignore stop-listening while connecting; keep pending manual start");
        return;
    } else if (state == kDeviceStateListening) {
        if (protocol_) {
...
void Application::SendRawJsonMessage(const std::string& payload) {
    Schedule([this, payload = std::move(payload)]() {
        if (protocol_) {
            protocol_->SendRawJson(payload);
        }
    });
}
