...
void WifiBoard::TryWifiConnect() {
    ...
    if (have_ssid) {
        ...
        // Do not block the startup task here, otherwise boards with no saved
        // credentials can trip the task watchdog before the main loop starts.
        ESP_LOGI(TAG, "No SSID configured, schedule WiFi config mode");
        xTaskCreate([](void* arg) {
            auto* board = static_cast<WifiBoard*>(arg);
            vTaskDelay(pdMS_TO_TICKS(1500));
            board->StartWifiConfigMode();
            vTaskDelete(NULL);
        }, "wifi_cfg_boot", 4096, this, 2, NULL);
    }
}
...
void WifiBoard::StartWifiConfigMode() {
    ...
    Application::GetInstance().Schedule([&wifi_manager]() {
        ...
        Application::GetInstance().Alert(Lang::Strings::WIFI_CONFIG_MODE, hint.c_str(), "wifi_ava", Lang::Sounds::OGG_WIFICONFIG);
    });
