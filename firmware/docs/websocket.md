...
```
- 设备侧结构里 `type` 可区分 `0: OPUS`、`1: JSON`；服务器当前仅处理 type=0 的音频负载。
- 若要发送控制类 JSON，请继续使用普通 WebSocket `text` / JSON 文本帧，不要依赖 `type=1` 的二进制封装。
...
```
- 版本3同样只有 `type=0` 的音频负载会被当前服务器路由；`type=1` 目前不会被当作 JSON 消息分发。
- 换句话说，二进制包负责音频，JSON 业务消息仍走 JSON 文本帧。
...
4. **Wake Word Detected**  
   ...
   - 如果该次 `listen.detect` 对应的是 AVE 界面内的“这个 / 买这个 / 帮我分析这个 / 这个能买吗”等指代性命令，设备端应同时附带当前界面的显式 `selection`，避免服务器使用陈旧游标或旧的 `current_token` 猜测目标。  
   - `selection` 是按次附带的命令上下文，不代表持续同步 UI 游标；没有可信 `selection` 时，服务器应对高风险指代命令 fail-closed，而不是静默回退到旧状态。  
   - 对真实 emitter 来说，只有本轮同时拿到 `selection.screen + selection.token.addr + selection.token.chain` 时才会发出 token 选择；如果缺 `selection.token.chain`，client 会直接省略整段 `selection`，而不是把缺失字段默认为 `solana`。  
   - 例：
     ```json
     {
       "session_id": "xxx",
     ...
     ```
   - `selection` 字段约定：
     - `screen`：当前 AVE 屏幕，如 `feed` / `portfolio` / `spotlight`
     - `cursor`：当前列表类界面的选中索引；详情页可省略
     - `token.addr` / `token.chain` / `token.symbol`：本次命令要操作的明确代币标识
     - 缺少 `selection.token.chain` 时，这次 listen payload 不应再携带 token 级 `selection`；服务端按 fail-closed 处理，不能把它默认为 `solana`
