...
class ConnectionHandler:
    def __init__(
        ...
        self.websocket_protocol_version = 1
        ...
        self._top_level_chat_lock = threading.RLock()
    ...
    async def handle_connection(self, ws: websockets.ServerConnection):
        try:
            ...
            self._record_websocket_protocol_version_from_headers()
    ...
    async def _save_and_close(self, ws):
        """保存记忆并关闭连接"""
        # Stop AVE real-time WSS tasks for this connection
        try:
            if hasattr(self, "ave_wss"):
                self.ave_wss.stop()
        except Exception as _e:
            self.logger.bind(tag=TAG).warning(f"AVE WSS stop error: {_e}")
    ...
    def _record_websocket_protocol_version(self, version):
        try:
            version = int(version)
        except (TypeError, ValueError):
            return
        if version > 0:
            self.websocket_protocol_version = version

    def _record_websocket_protocol_version_from_headers(self):
        ...
        if protocol_version is None:
            for key, value in headers.items():
                if isinstance(key, str) and key.lower() == "protocol-version":
                    protocol_version = value
                    break
        self._record_websocket_protocol_version(protocol_version)

    def _route_firmware_audio_message(self, message):
        protocol_version = getattr(self, "websocket_protocol_version", 1)
        ...
        if protocol_version == 2:
            if len(message) < 16:
                return False
            packet_version = int.from_bytes(message[0:2], "big")
            packet_type = int.from_bytes(message[2:4], "big")
            _reserved = int.from_bytes(message[4:8], "big")
            timestamp = int.from_bytes(message[8:12], "big")
            payload_size = int.from_bytes(message[12:16], "big")
            if packet_version != 2:
                return False
            packet_end = 16 + payload_size
            if len(message) < packet_end:
                return False
            if packet_type == 0:
                payload = message[16:packet_end]
        ...
        if protocol_version == 3:
            ...
            packet_end = 4 + payload_size
            if len(message) < packet_end:
                return False
            if packet_type == 0:
                payload = message[4:packet_end]
    ...
    async def _route_message(self, message):
        ...
        if isinstance(message, str):
            ...
            if self._route_firmware_audio_message(message):
                return
    ...
    def _get_top_level_chat_lock(self):
        chat_lock = getattr(self, "_top_level_chat_lock", None)
        if chat_lock is None:
            chat_lock = threading.RLock()
            self._top_level_chat_lock = chat_lock
        return chat_lock

    def chat(self, query, depth=0, ave_context=None):
        chat_lock = None
        if depth == 0:
            chat_lock = ConnectionHandler._get_top_level_chat_lock(self)
            chat_lock.acquire()

        try:
            turn_ave_context = ave_context
            if depth == 0:
                if turn_ave_context is None:
                    turn_ave_context = getattr(self, "ave_context", None)
                if isinstance(turn_ave_context, dict):
                    turn_ave_context = copy.deepcopy(turn_ave_context)
            ...
            if depth == 0:
                ...
                ave_context_message = self._build_ave_context_prompt(turn_ave_context)
                if ave_context_message:
                    self.dialogue.put(
                        Message(
                            role="user",
                            content=ave_context_message,
                            is_temporary=True,
                        )
                    )
            ...
            try:
                ...
                if depth == 0:
                    self._clear_temporary_dialogue_messages()
                return None
    ...
    def _clear_temporary_dialogue_messages(self):
        if len(self.dialogue.dialogue) == 0:
            return

        original_length = len(self.dialogue.dialogue)
        self.dialogue.dialogue = [
            msg for msg in self.dialogue.dialogue if not getattr(msg, "is_temporary", False)
        ]
        if len(self.dialogue.dialogue) < original_length:
            self.logger.bind(tag=TAG).debug("已清理临时上下文消息")

    def _build_ave_context_prompt(self, ave_context=None) -> str:
        if ave_context is None:
            ave_context = getattr(self, "ave_context", None)
        if not isinstance(ave_context, dict) or not ave_context:
            return ""

        context_text = json.dumps(ave_context, ensure_ascii=False)
        return (
            "[AVE_CONTEXT]"
            + context_text
            + "[/AVE_CONTEXT]"
            + " 请结合该设备界面状态理解“这个/确认/取消”等指代。"
        )
    ...
    def _handle_function_result(self, tool_results, depth, ave_context=None):
        need_llm_tools = []
        ...
        if need_llm_tools:
            ...
            if ave_context is None:
                ave_context = getattr(self, "ave_context", None)
            self.chat(None, depth=depth + 1, ave_context=ave_context)
