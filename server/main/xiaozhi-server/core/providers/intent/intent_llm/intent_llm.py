...
class IntentProvider(IntentProviderBase):
    ...
    def replyResult(
        ...
        text: str,
        original_text: str,
        ave_context: Optional[Dict] = None,
    ):
        try:
            system_prompt = text
            ave_context_prompt = self._build_ave_context_prompt(None, ave_context)
            if ave_context_prompt:
                system_prompt = f"{ave_context_prompt}\n{text}"
    ...
    def _build_ave_context_prompt(
        self, conn: "ConnectionHandler", ave_context: Optional[Dict]
    ) -> str:
        if not isinstance(ave_context, dict) or not ave_context:
            return ""

        builder = getattr(conn, "_build_ave_context_prompt", None)
        if callable(builder):
            return builder(ave_context)
    ...
    def _build_history_cache_key(self, dialogue_history: List[Dict]) -> str:
        ...
        return json.dumps(history_window, ensure_ascii=False, sort_keys=True)
    ...
    async def detect_intent(
        ...
        # 计算缓存键
        ave_context_prompt = self._build_ave_context_prompt(conn, ave_context)
        ave_context_key = ""
        if ave_context_prompt:
            ave_context_key = json.dumps(
                ave_context,
                ensure_ascii=False,
                sort_keys=True,
            )
        history_key = self._build_history_cache_key(dialogue_history)

        cache_key = hashlib.md5(
            (conn.device_id + text + ave_context_key + history_key).encode("utf-8")
        ).hexdigest()
        ...
        msgStr += f"User: {text}\n"
        if ave_context_prompt:
            user_prompt = f"{ave_context_prompt}\ncurrent dialogue:\n{msgStr}"
        else:
