...
async def handleHelloMessage(conn: "ConnectionHandler", msg_json):
    """处理hello消息"""
    conn._record_websocket_protocol_version(msg_json.get("version"))
    ...
    # ── AVE real-time WebSocket manager ──────────────────────────────────────
    try:
        from plugins_func.functions.ave_wss import AveWssManager, initial_feed_push
        conn.ave_wss = AveWssManager(conn)
        conn.ave_wss.start()
        # Push initial trending feed in background (device sees data immediately)
        asyncio.create_task(initial_feed_push(conn))
    except Exception as _ave_err:
        conn.logger.bind(tag=TAG).warning(f"AVE WSS manager start failed: {_ave_err}")
