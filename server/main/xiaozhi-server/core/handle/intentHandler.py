...
async def handle_user_intent(conn: "ConnectionHandler", text, ave_context=None):
    # 预处理输入文本，处理可能的JSON格式
    try:
    ...
    if conn.intent_type == "function_call":
        ...
        return False
    # 使用LLM进行意图分析
    intent_result = await analyze_intent_with_llm(conn, text, ave_context=ave_context)
    if not intent_result:
        return False
    # 会话开始时生成sentence_id
    conn.sentence_id = str(uuid.uuid4().hex)
    # 处理各种意图
    return await process_intent_result(conn, intent_result, text, ave_context=ave_context)
...
async def analyze_intent_with_llm(conn: "ConnectionHandler", text, ave_context=None):
    """使用LLM分析用户意图"""
    ...
    if ave_context is None:
        ave_context = getattr(conn, "ave_context", None)
    try:
...
async def process_intent_result(
    conn: "ConnectionHandler", intent_result, original_text, ave_context=None
):
    """处理意图识别结果"""
    turn_ave_context = copy.deepcopy(ave_context) if isinstance(ave_context, dict) else ave_context
    turn_sentence_id = getattr(conn, "sentence_id", None)
    if not turn_sentence_id:
        turn_sentence_id = str(uuid.uuid4().hex)
        conn.sentence_id = turn_sentence_id
    try:
        # 尝试将结果解析为JSON
        ...
        if "function_call" in intent_data:
            ...
            if function_name == "result_for_context":
                ...
                def process_context_result():
                    ...
                    speak_txt(conn, response, sentence_id=turn_sentence_id)
            ...
            def process_function_call():
                ...
                if result:
                    ...
                    if result.action == Action.RESPONSE:  # 直接回复前端
                        ...
                        if text is not None:
                            speak_txt(conn, text, sentence_id=turn_sentence_id)
                        ...
                        if llm_result is None:
                            llm_result = text
                        speak_txt(conn, llm_result, sentence_id=turn_sentence_id)
                    elif (
...
def speak_txt(conn: "ConnectionHandler", text, sentence_id=None):
    # 记录文本
