...
from core.handle.textHandler.aveCommandRouter import try_route_ave_command
        ...
        if msg_json["state"] == "start":
            ...
            if "text" in msg_json:
                ...
                if is_wakeup_words and not enable_greeting:
                    ...
                    # 先尝试确定性 AVE 路由；未命中再进入常规聊天。
                    if await try_route_ave_command(conn, original_text, msg_json):
                        return
                    # 否则需要LLM对文字内容进行答复
                    await startToChat(conn, original_text, message_payload=msg_json)
