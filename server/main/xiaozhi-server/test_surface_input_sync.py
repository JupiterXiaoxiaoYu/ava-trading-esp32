...
"""
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            disambiguation_stub_path = tmpdir_path / "screen_disambiguation_stubs.c"
            source_path.write_text(harness_source, encoding="utf-8")
            use_disambiguation_stub = (
                "screen_disambiguation_show" not in harness_source
                and not any(Path(source).name == "screen_disambiguation.c" for source in extra_sources)
            )
            if use_disambiguation_stub:
                disambiguation_stub_path.write_text(disambiguation_stub_source, encoding="utf-8")
            ...
            compile_result = subprocess.run(
                [
                    ...
                    str(source_path),
                    str(manager_src),
                    *([str(disambiguation_stub_path)] if use_disambiguation_stub else []),
                    *[str(source) for source in extra_sources],
                    *extra_ldflags,
                    "-o",
                    str(binary_path),
    ...
    def _verify_disambiguation_selection_payload(self):
        ...
        # Task 1 explicitly requires the disambiguation surface to emit no trusted
        # selection (so `selection` should either be absent or only include cursor metadata)
