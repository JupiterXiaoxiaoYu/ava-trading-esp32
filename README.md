# AVE Trading ESP32 Monorepo

(English | [中文](README_zh.md))

This repository is the active monorepo for the AVE hardware + backend + simulator stack.
...
- `server/` - backend stack, management services, AVE routing/tool logic, deployment docs, and server-side tests
- `shared/` - shared AVE LVGL screens that are compiled into both the firmware and the simulator
- `simulator/` - desktop validation harness for the shared AVE UI and mock interaction flows
...
- Work on server-side AVE behavior: start with [`server/README_en.md`](server/README_en.md) and [`server/main/README_en.md`](server/main/README_en.md)
- Preview or debug AVE pages on desktop: start with [`simulator/README.md`](simulator/README.md) and [`shared/ave_screens/README.md`](shared/ave_screens/README.md)
- Understand the shared UI contract: start with [`shared/README.md`](shared/README.md)
...
speech + input
  ...
  -> server/main/xiaozhi-server/ (ASR, routing, tools, AVE backend logic)
...
- [`shared/README.md`](shared/README.md) - how the cross-target AVE UI is organized
- [`shared/ave_screens/README.md`](shared/ave_screens/README.md) - screen files, manager, utilities, and extension workflow
- [`simulator/README.md`](simulator/README.md) - desktop build/run flow for the AVE simulator

### Device runtime

- [`firmware/README.md`](firmware/README.md) - upstream firmware capabilities plus AVE monorepo notes
...
- [`server/README_en.md`](server/README_en.md) - backend deployment overview with AVE monorepo framing
...
This monorepo is AVE-first, but several major directories are derived from upstream projects:
...
The READMEs in this repo describe those folders in terms of how AVE uses and customizes them today.
