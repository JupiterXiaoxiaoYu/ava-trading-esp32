...
│                                                                  │
│  HTTP Gateway      Legacy WS Gateway       Runtime Manager        │
│  /device/*         /ava/v1/               session per device      │
│  /admin/*          old firmware frames    outbox + event log      │
│                                                                  │
...
| `ava-devicekit/backend/ava_devicekit/gateway/legacy_firmware.py` | 兼容既有固件的 WebSocket 协议桥 |
...
| `ava-devicekit/schemas/` | manifest、screen payload、input event、context snapshot、action draft 合约 |
...
| Legacy WebSocket Gateway | 兼容既有固件 hello/listen/key_action 帧 | WebSocket text/binary/audio frames | display/TTS/ACK/command frames |
| Runtime Server | 单进程运行 HTTP + WS + background tasks | CLI `run-server` | 8788 HTTP + 8787 WS |
| Legacy Firmware Bridge | 把旧固件 wire protocol 转成 DeviceKit `DeviceMessage` | `hello`、`listen`、`key_action`、audio | app session handle 结果 |
...
| WebSocket legacy gateway | hello/device metadata 或默认 session |
...
| cursor 变化后 context 要更新 | 否则收藏/买入/介绍会作用到旧对象 |
...
| HTTP device API | 支持 |
| Legacy WebSocket firmware bridge | 支持 |
