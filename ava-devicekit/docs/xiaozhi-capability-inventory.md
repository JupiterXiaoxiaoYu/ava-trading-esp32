# Legacy Xiaozhi Capability Inventory

This document is the review checklist before migrating more legacy runtime code into Ava DeviceKit. Nothing in `ava-devicekit/` should depend on xiaozhi modules directly; each capability must be explicitly marked as `keep`, `replace`, `drop`, or `later` before implementation.
...
| `replace` | Keep the product behavior but replace the xiaozhi implementation shape |
...
| Capability | Legacy source | Proposed decision | DeviceKit target | Notes to confirm |
|---|---|---|---|---|
| WebSocket server | `server/main/xiaozhi-server/core/websocket_server.py` | `replace` | `gateway/websocket_server.py` | Keep transport role, replace connection/tool lifecycle |
...
| Prompt manager | `core/utils/prompt_manager.py` | `replace` | app manifest + model routing policy | Do not keep xiaozhi config shape |
...
| Config YAML | `xiaozhi-server/config.yaml` | `replace` | app manifest + deployment env | Do not expose xiaozhi config as framework API |
| `.env` model/API config | `xiaozhi-server/.env` | `keep` | deployment env | Keep env-driven secrets, rename around DeviceKit |
...
| ASR/LLM/TTS providers | Keep Qwen ASR + OpenAI-compatible LLM/TTS first | Port all xiaozhi providers, or use one vendor only |
