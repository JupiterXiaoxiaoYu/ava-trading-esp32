...
class PortfolioSkill:
    ...
    def orders(self, *, context: AppContext | None = None) -> ScreenPayload:
        rows = [row for row in _state(self.store).get("paper_orders", []) if isinstance(row, dict)]
        return ScreenPayload("portfolio", {"items": rows, "chain": SOLANA, "mode": "orders"}, context)
