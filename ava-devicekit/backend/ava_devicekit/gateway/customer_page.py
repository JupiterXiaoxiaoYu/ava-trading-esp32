...
<main class="shell">
  <section class="story">
    <div>
      ...
      <h1>Ava Device Activation</h1>
      <p class="lead">Sign in with the email used for your device, verify the activation code from the seller, and keep your Ava hardware connected to the service.</p>
    </div>
    <div class="steps" aria-label="Activation steps">
      <div class="step"><b>1</b><span>Use your customer email to create or resume a session.</span></div>
      <div class="step"><b>2</b><span>Enter the activation code shipped with the hardware.</span></div>
      <div class="step"><b>3</b><span>Confirm that your device appears under your account.</span></div>
    </div>
  </section>
  <aside class="panel">
    <section class="card">
      <div class="kicker">Customer sign in</div>
      <div class="title">Access your device</div>
      <p class="copy">No admin token is required here. This page is for hardware owners, not service operators.</p>
      <form id="customer-login-form" class="form">
        <div class="field"><label>Email</label><input name="email" type="email" autocomplete="email" required placeholder="you@example.com"></div>
        <div class="field"><label>Name</label><input name="display_name" autocomplete="name" placeholder="Optional"></div>
        <div class="row">
          <div class="field"><label>Wallet</label><input name="wallet" placeholder="Optional"></div>
          <div class="field"><label>App</label><input name="app_id" value="ava_box"></div>
        </div>
        <div class="actions"><button class="btn" type="submit">Sign in</button><button class="btn secondary" type="button" id="restore-session">Use saved session</button></div>
    ...
    <section class="card">
      ...
      <form id="activation-form" class="form">
        <div class="field"><label>Activation code</label><input name="activation_code" autocomplete="one-time-code" placeholder="AVA-0000-0000" required></div>
...
<script>
const $=s=>document.querySelector(s);const state={token:localStorage.getItem('ava_customer_token')||'',profile:null};
...
function renderProfile(body){state.profile=body;const c=body.customer||{};$('#profile-summary').textContent=c.email?`${c.display_name||c.email} - ${c.email}`:'Sign in to view your account.';const devices=body.devices||[];$('#device-list').innerHTML=devices.length?devices.map(d=>`<div class="device"><div class="device-head"><b>${esc(d.name||d.device_id)}</b><span class="pill ${['active','online_seen'].includes(d.status)?'ok':''}">${esc(d.status||'unknown')}</span></div><div class="meta"><span>ID ${esc(d.device_id||'-')}</span><span>App ${esc(d.app_id||'-')}</span><span>Board ${esc(d.board_model||'-')}</span><span>Firmware ${esc(d.firmware_version||'-')}</span></div></div>`).join(''):'<div class="empty">No device bound yet. Enter your activation code above.</div>'}
function esc(v){return String(v??'').replace(/[&<>"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]))}
async function me(){const body=await api('/customer/me');renderProfile(body);return body}
$('#customer-login-form').addEventListener('submit',async e=>{e.preventDefault();const body=Object.fromEntries(new FormData(e.target));Object.keys(body).forEach(k=>{if(body[k]==='')delete body[k]});try{const result=await api('/customer/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});state.token=result.customer_token;localStorage.setItem('ava_customer_token',state.token);renderProfile(result);setStatus('#login-status','Signed in. You can activate hardware now.')}catch(err){setStatus('#login-status',err.message,true)}});
$('#activation-form').addEventListener('submit',async e=>{e.preventDefault();const body=Object.fromEntries(new FormData(e.target));try{const result=await api('/customer/activate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});renderProfile(result);setStatus('#activation-status','Device activated and bound to this account.');e.target.reset()}catch(err){setStatus('#activation-status',err.message,true)}});
