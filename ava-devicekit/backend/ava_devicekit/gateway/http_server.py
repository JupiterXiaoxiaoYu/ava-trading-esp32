...
<main class="shell">
  ...
  <nav class="toolbar" aria-label="Admin sections">
    <button class="tab active" data-tab="overview">Overview</button><button class="tab" data-tab="control">Fleet Setup</button><button class="tab" data-tab="customers">Customers</button><button class="tab" data-tab="providers">Providers</button><button class="tab" data-tab="usage">Usage</button><button class="tab" data-tab="devices">Live Sessions</button><button class="tab" data-tab="firmware">Firmware</button><button class="tab" data-tab="services">Services</button><button class="tab" data-tab="events">Server Timeline</button><button class="tab" data-tab="raw">Raw</button>
  ...
  <section id="overview" class="panel active"><div class="grid"><div class="card wide"><h2>Service posture</h2><p class="sub">Server-wide view for one builder operating C-end hardware devices. Runtime events are server events across all devices unless filtered.</p><div id="overview-body" class="stack"></div></div><div class="card"><h3>Server timeline</h3><div id="overview-events" class="log"></div></div><div class="card"><h3>Operating flow</h3><div class="stack"><span class="pill info">1 configure providers</span><span class="pill info">2 create service plan</span><span class="pill info">3 provision device</span><span class="pill info">4 customer activates</span><span class="pill info">5 monitor usage</span></div><p class="sub">Team users operate the service. C-end customers bind purchased devices through activation codes.</p></div></div></section>
  ...
  <section id="usage" class="panel"><div class="grid"><div class="card wide"><h2>Service plans and usage</h2><p class="sub">Manual entitlement and cost-control layer for a builder operating C-end hardware service. Usage reports can come from devices or backend provider instrumentation.</p><div id="usage-summary" class="stack"></div></div><div class="card"><h3>Create service plan</h3><form id="plan-form" class="form"><div class="field"><label>Plan id</label><input name="plan_id" placeholder="plan_pro"></div><div class="field"><label>Name</label><input name="name" placeholder="Pro"></div><div class="field"><label>Price label</label><input name="price_label" placeholder="manual"></div><div class="field"><label>ASR seconds</label><input name="asr_seconds" type="number" min="0" placeholder="3600"></div><div class="field"><label>LLM tokens</label><input name="llm_tokens" type="number" min="0" placeholder="200000"></div><div class="field"><label>TTS chars</label><input name="tts_chars" type="number" min="0" placeholder="200000"></div><div class="field"><label>API calls</label><input name="api_calls" type="number" min="0" placeholder="10000"></div><button class="btn" type="submit">Create plan</button></form></div><div class="card"><h3>Assign entitlement</h3><form id="entitlement-form" class="form"><div class="field"><label>Device id</label><input name="device_id" placeholder="ava_box_001" required></div><div class="field"><label>Plan id</label><input name="plan_id" placeholder="plan_starter" required></div><div class="field"><label>Status</label><select name="status"><option>active</option><option>trial</option><option>suspended</option><option>expired</option><option>lifetime</option></select></div><div class="field"><label>Expires at unix</label><input name="expires_at" type="number" min="0" placeholder="0"></div><button class="btn warn" type="submit">Assign</button></form></div><div class="card"><h3>Record usage</h3><form id="usage-form" class="form"><div class="field"><label>Device id</label><input name="device_id" placeholder="ava_box_001" required></div><div class="field"><label>Metric</label><select name="metric"><option>asr_seconds</option><option>llm_tokens</option><option>tts_chars</option><option>api_calls</option></select></div><div class="field"><label>Amount</label><input name="amount" type="number" min="0" step="0.01" required></div><div class="field"><label>Source</label><input name="source" placeholder="admin"></div><button class="btn" type="submit">Record</button></form></div><div class="card"><h3>Service plans</h3><div id="plans-table"></div></div><div class="card wide"><h3>Usage by device</h3><div id="usage-table"></div></div></div></section>
  ...
  <section id="events" class="panel"><div class="card wide"><h2>Server timeline</h2><p class="sub">This is backend runtime information across all devices. Use device id to narrow it to one hardware unit.</p><form id="events-form" class="form"><div class="field"><label>Device id</label><input name="device_id" placeholder="optional"></div><div class="field"><label>Event type</label><input name="event" placeholder="runtime.error"></div><div class="field"><label>Limit</label><input name="limit" value="100"></div><button class="btn" type="submit">Filter</button></form><div id="events-log" class="log"></div></div></section>
...
<script>
const state={dashboard:null,usage:null};
...
function render(){const d=state.dashboard;if(!d)return; const devices=d.devices?.items||[], fw=d.firmware?.items||[], providers=d.providers?.items||[], services=d.developer_services?.items||[], events=d.events?.items||[], cp=d.control_plane||{}, customers=cp.customers||[], plans=cp.service_plans||[], usage=state.usage||{}; const online=devices.filter(x=>x.connection&&x.connection.connected).length; $('#m-devices').textContent=cp.counts?.provisioned_devices??devices.length; $('#m-online').textContent=online; $('#m-firmware').textContent=fw.length; $('#m-providers').textContent=d.providers?.ok?'yes':'check'; $('#raw-json').textContent=JSON.stringify(d,null,2);
 ...
 $('#control-summary').innerHTML=[info('operators '+(cp.counts?.users??0)),info('customers '+(cp.counts?.customers??0)),info('projects '+(cp.counts?.projects??0)),info('provisioned '+(cp.counts?.provisioned_devices??0)),info('registered '+(cp.counts?.registered_devices??0)),info('active '+(cp.counts?.active_devices??0))].join(' ');
 ...
 $('#usage-summary').innerHTML=[info('period '+(usage.period||cp.usage_summary?.period||'-')),info('plans '+plans.length),info('asr '+(cp.usage_summary?.totals?.asr_seconds??0)),info('llm '+(cp.usage_summary?.totals?.llm_tokens??0)),info('tts '+(cp.usage_summary?.totals?.tts_chars??0)),info('api '+(cp.usage_summary?.totals?.api_calls??0))].join(' ');
 $('#plans-table').innerHTML=table([{label:'Plan',render:r=>`<span class="mono">${escapeHtml(r.plan_id)}</span>`},{label:'Name',key:'name'},{label:'Price',key:'price_label'},{label:'Limits',render:r=>`<span class="mono">${escapeHtml(JSON.stringify(r.limits||{}))}</span>`},{label:'Status',render:r=>info(r.status||'-')}],plans,'No service plans.');
 $('#usage-table').innerHTML=table([{label:'Device',render:r=>`<span class="mono">${escapeHtml(r.device_id)}</span>`},{label:'Customer',render:r=>`<span class="mono">${escapeHtml(r.customer_id||'-')}</span>`},{label:'Plan',render:r=>info(r.entitlement?.plan_id||'none')},{label:'Usage',render:r=>`<span class="mono">${escapeHtml(JSON.stringify(r.usage||{}))}</span>`},{label:'Limit',render:r=>pill(!!(r.limit_status&&r.limit_status.ok),r.limit_status?.reason||'-')}],usage.items||[],'No usage recorded yet.');
...
async function refresh(){try{state.dashboard=await api('/admin/dashboard.json'); state.usage=await api('/admin/usage'); render(); toast('Dashboard refreshed')}catch(e){toast('Refresh failed: '+e.message)}}
...
$('#plan-form').addEventListener('submit',async e=>{e.preventDefault();const fd=new FormData(e.target);const body={plan_id:fd.get('plan_id')||'',name:fd.get('name')||'',price_label:fd.get('price_label')||'',limits:{asr_seconds:Number(fd.get('asr_seconds')||0),llm_tokens:Number(fd.get('llm_tokens')||0),tts_chars:Number(fd.get('tts_chars')||0),api_calls:Number(fd.get('api_calls')||0)}};try{await api('/admin/service-plans',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});toast('Service plan created');e.target.reset();await refresh()}catch(err){toast('Create plan failed: '+err.message)}});
$('#entitlement-form').addEventListener('submit',async e=>{e.preventDefault();const fd=new FormData(e.target);const id=fd.get('device_id');const body={plan_id:fd.get('plan_id'),status:fd.get('status'),expires_at:Number(fd.get('expires_at')||0)};try{await api(`/admin/devices/${encodeURIComponent(id)}/entitlement`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});toast('Entitlement assigned');e.target.reset();await refresh()}catch(err){toast('Entitlement failed: '+err.message)}});
$('#usage-form').addEventListener('submit',async e=>{e.preventDefault();const fd=new FormData(e.target);const body={device_id:fd.get('device_id'),metric:fd.get('metric'),amount:Number(fd.get('amount')||0),source:fd.get('source')||'admin'};try{await api('/admin/usage',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});toast('Usage recorded');e.target.reset();await refresh()}catch(err){toast('Usage failed: '+err.message)}});
